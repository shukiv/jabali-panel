...
// The wildcard matcher itself (x509.VerifyHostname semantics) is now the
// domainops leaf, table-tested in internal/domainops/sharedcert_test.go. This
// door keeps only TestSharedCertCoversHost as the proof that api's
// sharedCertCoversHost still routes through that leaf.
