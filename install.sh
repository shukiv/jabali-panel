...
CADDYEOF
    ...
    if [ -n "${addon_blocks}" ]; then
        ...
        # Overwrite in-place (not mv) so file perms/owner of /etc/jabali/Caddyfile
        # stay as generated by the heredoc (0644 root:root, readable by www-data).
        cat "${tmp_caddyfile}" > /etc/jabali/Caddyfile
        rm -f "${tmp_caddyfile}"
