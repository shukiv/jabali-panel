...
| JAB-190: Billing automation endpoints (WHMCS/Blesta/WiseCP) | 2026-08-08 | ADR-0164; blueprint `plans/billing-automation-endpoints.md`. Account lifecycle on the M44/JAB-140 Automation API for the `jabali-integration` billing modules: `POST/DELETE /automation/users` (delete = first `delete:*` verb per the ADR-0157 reservation, `confirm:true` + `dry_run` preview, admin targets refused), `POST …/password`, `PUT …/package`, snapshot-only usage reads (`GET …/usage`, bulk `GET /automation/usage`), `GET /automation/packages` (new `read:packages` scope), `?email=/?username=` user lookup filters. GUI password/package/delete-cascade logic extracted to `userops` (`RotatePassword`/`SetPackage`/`DeleteCascade`) so REST + automation share one write path. Login-token SSO shipped as ADR-0165: `POST …/login-token` mints a single-use Kratos admin recovery link (`users.login_token` capability) — no bespoke session machinery. Live-verified on mx (branch build deploy → 15/15 lifecycle + 6/6 SSO smoke checks through the shared PHP client, box rolled back after). On branch `feat-billing-automation`, pending dispatcher merge. |

---

**Last updated:** 2026-08-08 (JAB-190 billing automation endpoints added)
